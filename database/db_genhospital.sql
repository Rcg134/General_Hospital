...
CREATE DEFINER=`root`@`localhost` PROCEDURE `Insert_profile` (IN `prof_firstname` VARCHAR(255), IN `prof_lastname` VARCHAR(255), IN `email` VARCHAR(255), IN `birthdate` DATE, IN `contact_number` VARCHAR(255), IN `bio` VARCHAR(255), IN `usertype` INT, IN `IID` INT, IN `specializeid` INT)   BEGIN
